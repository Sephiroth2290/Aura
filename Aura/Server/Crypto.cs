...
namespace Aura.Server
{
  ...
  public class Crypto
    {
        private byte[] m_key;
        private string m_keySalt;
        private static byte[][] m_salts;
        private byte m_seed;
        ...
        internal byte[] Salt
        {
            get
            {
                return Crypto.m_salts[(int)this.m_seed];
            }
        }
       ...
       0x80, 0x7F, 0x81, 0x7E, 0x82, 0x7D, 0x83, 0x7C, 0x84, 0x7B, 0x85, 0x7A, 0x86, 0x79, 0x87, 0x78,
        ...
        0xFF, 0xFE, 0xFD, 0xFC, 0xFB, 0xFA, 0xF9, 0xF8, 0xF7, 0xF6, 0xF5, 0xF4, 0xF3, 0xF2, 0xF1, 0xF0,
         ...
         0xFF, 0x1E, 0x1E, 0x1E, 0x1E, 0x1E, 0x1E, 0x1E, 0x1E, 0x3B, 0x3B, 0x3B, 0x3B, 0x3B, 0x3B, 0x3B,
            ...
            Crypto.m_salts = numArray1;
        }

        internal Crypto(byte seed, string key, string keySalt = null)
        {
            this.m_seed = seed;
            this.m_key = Encoding.ASCII.GetBytes(key);
            this.m_keySalt = keySalt;
        }
        ...
        internal byte[] GenerateKey(ushort a, byte b)
        {
            ...
            {
                int index2 = (index1 * (9 * index1 + (int)b * (int)b) + (int)a) % this.m_keySalt.Length;
                numArray[index1] = (byte)this.m_keySalt[index2];
            }
            return numArray;
        }
        ...
        public static string GenerateKeySalt(string seed)
        {
            ...
            for (int index = 0; index < 31; ++index)
                hashString += GetHashString(hashString, "MD5");
            return hashString;
        }

        internal static string GetHashString(string value, string hashName)
        {
            return BitConverter.ToString(HashAlgorithm.Create(hashName).ComputeHash(Encoding.ASCII.GetBytes(value))).Replace("-", string.Empty).ToLower();
        }
    }
